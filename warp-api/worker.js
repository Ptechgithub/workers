...
async function registerDevice(publicKey, installId = '') {
  const payload = {
...
function generateWireGuardURL({PrivateKey, PeerPublicKey, Reserved, IPv6, ServerIP, Port}) {
  ...
  const url = `wireguard://${encodeURIComponent(PrivateKey)}@${ServerIP}:${Port}?address=${encodeURIComponent(IPv4)},${encodeURIComponent(IPv6)}&presharedkey=&reserved=${encodeURIComponent(ReservedStr)}&publickey=${encodeURIComponent(PeerPublicKey)}&mtu=1420#P_WG`;
  return url;
}
...
async function handleRequest(request) {
  try {
      ...
      const reserved = Array.from(fromBase64(clientId).slice(0, 3));
      ...
      if (pathname === '/' || pathname === '/help') {
          const helpText = `
Cloudflare WARP WireGuard API Worker
-------------------------------------
By → Peyman
https://github.com/Ptechgithub.
-------------------------------------
...
`.trim();
      ...
      return new Response(`Error: ${err.stack || err}`, {
          status: 500,
          headers: {'Content-Type': 'text/plain'},
      });
  }
